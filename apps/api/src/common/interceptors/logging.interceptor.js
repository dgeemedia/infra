...
var LoggingInterceptor = function () {
    ...
    var LoggingInterceptor = _classThis = /** @class */ (function () {
        ...
        LoggingInterceptor_1.prototype.intercept = function (context, next) {
            ...
            var partnerId = (_b = (_a = request.partner) === null || _a === void 0 ? void 0 : _a.id) !== null && _b !== void 0 ? _b : 'anonymous';
            var startTime = Date.now();
            return next.handle().pipe((0, rxjs_1.tap)({
                next: function () {
                    var response = context.switchToHttp().getResponse();
                    var duration = Date.now() - startTime;
                    _this.logger.log("".concat(method, " ").concat(url, " \u2192 ").concat(response.statusCode, " [").concat(duration, "ms] partner=").concat(partnerId));
                },
                error: function (error) {
                    var duration = Date.now() - startTime;
                    _this.logger.warn("".concat(method, " ").concat(url, " \u2192 ERROR [").concat(duration, "ms] partner=").concat(partnerId, " err=").concat(error.message));
                },
            }));
        };
