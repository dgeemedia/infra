...
export default function SettingsPage() {
  ...
  // ── Self-suspend ──────────────────────────────────────────
  // Calls PATCH /v1/partners/me/suspend — uses the partner's own
  // dashboard JWT. Does NOT require AdminGuard. This is the fix —
  // the old version called /v1/admin/partners/:id/suspend which
  // requires role=ADMIN and therefore always returned 403.
  const selfSuspendMutation = useMutation({
    mutationFn: async () => {
      await api.patch('/v1/partners/me/suspend');
    },
    onSuccess: async () => {
    ...
    <div className="space-y-6 max-w-2xl">
      ...
      <div className="rounded-2xl border border-destructive/30 bg-destructive/5 p-6 space-y-4">
        ...
        <p className="text-sm text-muted-foreground">
          ...
          {' '}— only Elorge admins can reactivate a suspended account.
        </p>
        <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-sm rounded-2xl bg-card p-6 shadow-xl">
            ...
            <p className="mt-2 text-center text-sm text-muted-foreground">
              ...
              You will be signed out. Only Elorge admins can reactivate your account —
              email <strong>support@elorge.com</strong>.
            </p>
            {suspendError && (
