...
import { formatNaira, formatDate, truncateId }                      from '@/lib/utils';
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">Every payout across all partners</p>
      </div>
      ...
      <div className="overflow-hidden rounded-xl border border-border bg-card">
            ...
            <tbody>
              ...
              {!isLoading && filtered.map((payout: AdminTransaction) => (
                <tr key={payout.id} className="border-b border-border last:border-0 hover:bg-muted/20 transition-colors">
                  <td className="px-4 py-3 font-mono text-xs">{truncateId(payout.partnerReference)}</td>
                  ...
                  <td className="px-4 py-3 font-medium text-foreground text-xs">{formatNaira(payout.nairaAmountKobo / 100)}</td>
                  <td className="px-4 py-3 text-xs text-muted-foreground">{formatDate(payout.createdAt)}</td>
        ...
        {data && data.totalPages > 1 && (
          <div className="flex items-center justify-between border-t border-border px-4 py-3">
            <p className="text-xs text-muted-foreground">{data.total} total transactions</p>
