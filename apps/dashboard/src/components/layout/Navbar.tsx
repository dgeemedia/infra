...
export function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-30 flex h-16 items-center border-b border-border bg-card/95 backdrop-blur px-4 gap-3">
      ...
      {/* Hamburger — always visible on mobile, shown on desktop when collapsed */}
      <button
        onClick={toggleSidebar}
        className={cn(
          'rounded-lg p-1.5 hover:bg-muted transition-colors',
          // On desktop only show when sidebar is collapsed
          sidebarOpen ? 'hidden lg:hidden' : 'flex lg:flex',
        )}
      ...
      <div ref={bellRef} className="relative">
        ...
        {bellOpen && (
          <div className="absolute right-0 top-full mt-2 w-80 max-w-[calc(100vw-2rem)] rounded-xl border border-border bg-card shadow-xl overflow-hidden">
                ...
                >
                  <div className="flex items-start gap-3">
                    <span className={cn(
                      ...
                      n.read ? 'bg-muted-foreground/30' : (TYPE_DOT[n.type] ?? 'bg-primary'),
                    )} />
