  ...
  return (
    <>
      ...
      <aside className={cn(
        ...
        // Mobile: slide in/out as a full drawer (always w-60 wide)
        // Desktop: collapse to icon-only w-16 or full w-60
        sidebarOpen
          ? 'w-60 translate-x-0'
        ...
        <div className="flex h-16 items-center border-b border-border px-4">
          ...
          {sidebarOpen && (
            ...
            >
              {/* On mobile show X, on desktop show chevron */}
              <X className="h-4 w-4 text-muted-foreground lg:hidden" />
              <ChevronLeft className="h-4 w-4 text-muted-foreground hidden lg:block" />
            </button>
          )}
        </div>
        ...
        <nav className="flex flex-col gap-1 p-2 pt-4 overflow-y-auto h-[calc(100vh-4rem-4rem)]">
          ...
          {isAdmin && (
            <>
              <div className={cn(
                'my-2',
                sidebarOpen ? 'border-t border-border pt-2' : 'flex justify-center',
              )}>
                {sidebarOpen && (
